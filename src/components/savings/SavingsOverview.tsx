...
export function SavingsOverview({ totalSaved, totalTarget, overallProgress, goalCount }: SavingsOverviewProps) {
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-savings/15 bg-savings-soft p-5 animate-fade-up">
      <div className="flex items-center gap-5">
        <div className="relative shrink-0">
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <span className="text-2xl font-bold text-ink tabular-nums">{overallProgress}%</span>
          </div>
        </div>
        <div>
          <p className="text-xs font-semibold uppercase tracking-wider text-savings/70">Total Saved</p>
          <p className="text-3xl font-bold text-ink tabular-nums font-display">{formatCurrency(totalSaved)}</p>
